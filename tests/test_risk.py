...
def _account(**overrides):
    values = dict(
        readable=True, equity=Decimal("100000"), balance=Decimal("100000"),
        margin_available=Decimal("50000"), daily_loss_so_far=Decimal("0"),
...
def _intent(stop: Decimal = Decimal("0.0010"), volume: Decimal = Decimal("1.0")):
    return OrderIntent(
        ...
        stop_loss=stop, take_profit=None, volume=volume,
    )
...
def test_val063_volume_rounds_down_risk_recomputed():
    ...
    decision = _engine().size_position(
        ...
        open_risk=Decimal(0), open_margin=Decimal(0),
        reservations=ReservationLedger(), stop_distance=Decimal("0.0010"), spec=_spec(),
        value_per_price_unit_per_lot=Decimal("100000"), gap_95=Decimal("0.0005"),
...
def test_val063_partial_fill_records_risk_from_filled_volume():
    ...
    decision = engine.try_reserve(
        ledger=ledger, open_risk=Decimal(0), open_margin=Decimal(0),
        key="order-1", max_loss=Decimal("2.30"), margin=Decimal("250"),
        equity=Decimal("100000"), now=dt.datetime(2024, 1, 2, tzinfo=UTC),
    )
...
def test_val063_rounded_volume_below_min_rejected_not_rounded_up():
    decision = _engine().size_position(
        account=_account(equity=Decimal("150"), balance=Decimal("150")),  # 0.5% = 1.0 -> volume under min
        open_risk=Decimal(0), open_margin=Decimal(0),
        reservations=ReservationLedger(), stop_distance=Decimal("0.0010"), spec=_spec(),
        value_per_price_unit_per_lot=Decimal("100000"), gap_95=Decimal("0.0005"),
        daily_limit=Decimal("5000"),
...
def test_val063_gap_adjusted_stop_widens_sizing_basis():
    ...
    decision = _engine().size_position(
        account=_account(), open_risk=Decimal(0), open_margin=Decimal(0),
        reservations=ReservationLedger(),
        stop_distance=Decimal("0.0010"), spec=_spec(),
...
def test_unreadable_state_is_a_veto_with_no_override():
    decision = _engine().size_position(
        account=_account(readable=False), open_risk=Decimal(0), open_margin=Decimal(0),
        reservations=ReservationLedger(), stop_distance=Decimal("0.0010"), spec=_spec(),
        value_per_price_unit_per_lot=Decimal("100000"), gap_95=Decimal("0.0005"),
        daily_limit=Decimal("5000"),
    )
...
def test_risk013_two_pending_entries_jointly_over_limits():
    ...
    first = engine.try_reserve(
        ledger=ledger, open_risk=Decimal(0), open_margin=Decimal(0),
        key="EURUSD-1", max_loss=Decimal("1200"), margin=Decimal("2500"),
        equity=Decimal("100000"), now=dt.datetime(2024, 1, 2, tzinfo=UTC),
    )
    assert first.accepted
    second = engine.try_reserve(
        ledger=ledger, open_risk=Decimal(0), open_margin=Decimal(0),
        key="USDJPY-1", max_loss=Decimal("1200"), margin=Decimal("2500"),
        equity=Decimal("100000"), now=dt.datetime(2024, 1, 2, tzinfo=UTC),
    )
...
def test_reservation_survives_timeout():
    ...
    ledger = ReservationLedger()
    ledger.reserve(Reservation(key="k", max_loss=Decimal("100"),
                               margin=Decimal("500"),
