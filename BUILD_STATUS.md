...
| Trailing-exit campaign (10%/90d @ DD<8%) | Trailing bracket in the lab engine (ratcheting SL + trailing TP, causal, unit-tested to the pip), TrailingMomentum strategy, `optimize_trailing.py` + `portfolio_trailing.py` | swept EURUSD/GBPUSD/XAUUSD 1h x 54 trailing configs with train/validate discipline; XAUUSD honestly rejected (3 validate trades); trailing exits improve stability (GBPUSD validate DD 1.25%) but cut the return tail; the +10%-per-90-days target at <8% DD is hit by the STATIC-RR books at 1.45x inverse-DD weighting: full DD 7.78%, median 90-day window +10.3% (best +17.1%), $10k -> $14,273 over 17.5 months |
| Expert Advisor (`ea/`) | `LondonMomentumPortfolio.mq5` — faithful MQL5 port of both accepted london_momentum books (entries, features, sizing, session/one-per-day gates, cost-veto stop floor) with live necessities (broker stops-level, spread guard, per-book daily lockout, Friday flat, margin cap) | compiles clean with the terminal's MetaEditor (`ea/LondonMomentumPortfolio.ex5`, 0 errors); installed into the LHFX demo terminal's `MQL5\\Experts`; NOT yet run live — attach to demo first, set `InpUTCOffsetHours` to the broker's UTC offset |
