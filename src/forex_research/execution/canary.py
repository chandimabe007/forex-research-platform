...
def persist_canary_record(record: CanaryRecord, *, out_dir: Path) -> Path:
    ...
    payload = {
        ...
        "steps": [
            {"name": s.name, "ok": s.ok, "detail": s.detail} for s in record.steps
        ],
...
def run_canary(
    ...
    *,
    ...
    steps.append(Step("connect", True, {
    ...
    try:
        ...
        steps.append(Step("session_status", status is SessionStatus.TRADES_ALLOWED,
                          {"status": status.value}))
    ...
    steps.append(Step("symbol_info", True, {
    ...
    try:
        ...
        return finish("failed", {})
    steps.append(Step("pre_trade_state", True, {
    ...
    if not result.ok:
        steps.append(Step("submit", False, {
    ...
    entry_slippage = (
        (fill_price - slippage_ref["decision_ask"]) if fill_price is not None else None
    )
    steps.append(Step("submit", True, {
    ...
    steps.append(Step("correlation_search", found is not None, {
    ...
    steps.append(Step("broker_side_protection", protection_ok, {
    ...
    if not close.ok:
        steps.append(Step("close", False, {
            "retcode": close.retcode, "comment": close.comment,
        }))
        discrepancies.append(
    ...
    steps.append(Step("reconcile", True, {
        "balance_before": balance_before,
